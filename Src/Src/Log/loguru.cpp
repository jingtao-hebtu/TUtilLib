        ...
        for (int arg_it = 1; arg_it < argc; ++arg_it) {
            ...
            if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !last_is_alpha) {
                ...
                if (req_verbosity != Verbosity_INVALID) {
                    ...
                    CHECK_F(end && *end == '\0',
                            "Invalid verbosity. Expected integer, INFO, WARNING, ERROR or OFF, got '" LOGURU_FMT(s) "'",
                            value_str);
                }
            } else {
...
#define getcwd _getcwd
    ...
    bool add_file(const char *path_in, FileMode mode, Verbosity verbosity) {
        ...
        VLOG_F(g_internal_verbosity,
               "Logging to '" LOGURU_FMT(s) "', mode: '" LOGURU_FMT(s) "', verbosity: " LOGURU_FMT(d) "", path,
               mode_str, verbosity);
        return true;
    }
    ...
    */
    ...
    static void print_preamble_header(char *out_buff, size_t out_buff_size) {
        ...
        if (g_preamble_thread && pos < out_buff_size) {
            int bytes = snprintf(out_buff + pos, out_buff_size - pos, "[%-*s]", LOGURU_THREADNAME_WIDTH,
                                 " thread name/id");
            if (bytes > 0) {
                pos += bytes;
            }
    ...
    static void
    print_preamble(char *out_buff, size_t out_buff_size, Verbosity verbosity, const char *file, unsigned line) {
        if (out_buff_size == 0) { return; }
        out_buff[0] = '\0';
    ...
    void
    log_and_abort(int stack_trace_skip, const char *expr, const char *file, unsigned line, const char *format, ...) {
        va_list vlist;
                va_start(vlist, format);
        auto buff = vtextprintf(format, vlist);
...
#else
                auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ",
                                         LOGURU_FILENAME_WIDTH, filename(entry->_file), entry->_line,
                                         description.c_str());
#endif
